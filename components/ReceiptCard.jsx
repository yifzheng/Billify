...
const ReceiptCard = ( { receipt } ) => {
    ...
    return (
        <section className={ `flex flex-col gap-2 bg-white rounded-lg shadow-lg p-6 h-${expanded ? 'auto' : 200} transition-max-full sm:w-1/3 w-full relative font-inter` }>
            <h2 className='sm:text-2xl text-xl font-semibold font-inter'>{ receipt.resturantName }</h2>
            <div className="flex justify-between">
                <p className='text-gray-700 text-lg font-inter'><b>Total:</b> ${ receipt.total }</p>
            </div>
                    ...
                    <section className="extra sm:text-base text-sm text-gray-700 font-inter ">
                        ...
                        <section className='flex justify-between items-start font-inter'>
                            ...
                            <section className='flex-end flex-col justify-between w-1/4 font-inter'>
                                ...
                                { receipt.contribution.map( ( item, index ) =>
                                (
                                    <span key={ index } className=''>${ item.contribution }</span>
                                ) ) }
                            </section>
                        </section>
                        <hr className='h-1 bg-black my-2' />
                        <section className='flex justify-between'>
                            ...
                            <span className='pr-4 font-inter'>${ receipt.tax }</span>
                        </section>
                        <section className='flex justify-between mb-7'>
                            <span className='font-inter'><b>Tip</b></span>
                            <span className='pr-4 font-inter'>${ receipt.tip }</span>
                        </section>
                    </section>
                ) }
        </section>
