...
const Form = ( { type } ) => {
    const [ items, setItems ] = useState( [
        {
            description: '', price: null, quantity: 1, owners: []
        }
    ] )

    // add another item to state
    const handleAddItem = () => {
        setItems( [ ...items, { description: '', price: null, quantity: 1, owners: [] } ] );
    };
    ...
    return (
        <section className='w-full max-w-full flex-start flex-col mb-16'>
            <h1 className='head_text text-left'><span className='orange_gradient'>{ type } Receipt</span></h1>
            <form
